...
//硬件看门狗
void IWDG_Init(u8 prer,u16 rlr);  //4,500 ( 4*2^prer*rlr )/32 == 1000 ms //这里按照32k计时,实际时间不准确,在15~47khz波动
void IWDG_Feed(void);		//硬件喂狗
...
void WWDG_Init(u8 tr,u8 wr,u8 fprer);    //WWDG_Init(0X7F,0X5F,3);	//计数器值为7f,窗口寄存器为5f,分频数为8	  
//重设置WWDG计数器的值 
void WWDG_Set_Counter(u8 cnt);
//需要自己实现的喂狗函数,使用窗口看门狗时,给cpu写遗嘱
extern void WWDG_CallBack(void);
